...
async def booking_confirmation2(data: BookingDataMail):
    ...
    replacements = {
        "{{ name }}": data.NAME or "",
        "{{checkindate}}": data.CHECKIN or "",
        "{{checkoutdate}}": data.CHECKOUT or "",
        "{{dayofcheckin}}": data.DAYOF_CHECKIN or "",
        "{{dayofcheckout}}": data.DAYOF_CHECKOUT11 or "",
        "{{no_of_night}}": data.NO_OF_NIGHTS or "",
        "{{checkintime}}": data.CHECK_IN_TIME or "",
        "{{checkouttime}}": data.CHECK_OUT_TIME or "",
        "{{hotelname}}": data.HOTELNAME or "",
        "{{hoteladdress}}": data.HOTELADDRESS or "",
        "{{hotelphone}}": str(data.HOTELPHONE) if data.HOTELPHONE else "",
        "{{noofrooms}}": data.ROOMCOUNT or "",
        "{{noofguest}}": data.GUESTCOUNT or "",
        "{{roomcharges}}": data.ROOM_CHARGES or "",
        "{{inclusions}}": data.INCLUSIONS or "",
        "{{gst}}": data.GST_VALUE or "",
        "{{SUBTOTAL}}": data.SUBTOTAL or "",
        "{{grandtotal}}": data.AMT_TO_BE_PAID or "",
        "{{PAYMENTMODE}}": data.PAYMENTMODE or "",
        "{{ADDON_POLICES}}": data.ADDON_POLICES or "",
        "{{DEFAULT_POLICES}}": data.DEFAULT_POLICES or "",
        "{{CANCELLATIONPOLICY}}": data.CANCELLATIONPOLICY or "",
        "{{EMPNAME}}": data.EMPNAME or "",
        "{{EMPPHONE}}": data.EMPPHONE or "",
        "{{EMPEMAIL}}": data.EMPEMAIL or "",
        "{{location}}": data.LOCATIONLINK or "",
        "{{GUESTTABLE}}": table,  
        "{{client}}": data.CLIENT or "",
        "{{clientgst}}": data.CLIENT_GST or "",
        "{{booking_date}}": data.Booking_Date or "",
        "{{booking_id}}": data.Booking_Id or "",
        "{{BRID}}": data.Brid or "",
        "{{GST_PRECENT}}": data.GST_PRECENT or ""
    }
